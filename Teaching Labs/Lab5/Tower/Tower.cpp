...
// ==== Remove the following 5 lines of code and replace with a 3D model! ====
    ...
    for(int j = 0; j < 1; j++) {
        ...
        for(int i = 1; i < 7; i++) {
            ...
            glTexCoord2f(s, 0);    glVertex3f(vx[i], vy[i], vz[i]);
            glTexCoord2f(s, 1);    glVertex3f(wx[i], wy[i], wz[i]);
            // Need to adjust for vertices 6 - 12 as they have closer spacing
        }
        for(int i = 7; i < 13; i++) {

            int i_temp = (i - 6);
            float s = 0.4 + (i_temp * (0.2 / 6));
            normal(vx[i-1], vy[i-1], vz[i-1], vx[i], vy[i], vz[i], wx[i], wy[i], wz[i]);
            glTexCoord2f(s, 0);    glVertex3f(vx[i], vy[i], vz[i]);
            glTexCoord2f(s, 1);    glVertex3f(wx[i], wy[i], wz[i]);
        }
        ...
        glTexCoord2f(1., 0.);   glVertex3f(vx[0], vy[0], vz[0]);
        glTexCoord2f(1., 1.);   glVertex3f(wx[0], wy[0], wz[0]);
        for(int i = 0; i < N; i++) {
            vx[i] = wx[i];
            vy[i] = wy[i];
            vz[i] = wz[i];
