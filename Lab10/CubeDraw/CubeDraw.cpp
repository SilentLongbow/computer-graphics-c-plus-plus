...
GLuint vaoID;
//GLuint texID; // One texture doesn't require an array
GLuint texID[2];
GLuint theProgram;
...
const float CDR = M_PI/180.0;   //Conversion from degrees to radians (required in GLM 0.9.6)

void loadTextures()
{
    // glGenTextures(1, &texID);    // One texture

    glGenTextures(2, texID);   //Generate 2 texture IDs
   ...
   // Load brick texture
    ...
    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);   // Must define both for EACH texture
...
void initialise()
{
	...
	GLuint texLoc1 = glGetUniformLocation(program, "tSampler1");    //tSampler1 is a uniform variable of type Sampler2D in the fragment shader
	glUniform1i(texLoc1, 0);    //Assign a value 0 to the variable tSampler1 to specify that it should use the texture from unit 0.

	GLuint texLoc2 = glGetUniformLocation(program, "tSampler2");    //tSampler2 is a uniform variable of type Sampler2D in the fragment shader
	glUniform1i(texLoc2, 1);    //Assign a value 1 to the variable tSampler2 to specify that it should use the texture from unit 1.
